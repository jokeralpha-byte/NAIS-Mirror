...
from model.inpainter.flow_comp import SPyNet
from model.inpainter.feat_prop import BidirectionalPropagation, SecondOrderDeformableAlignment
from model.inpainter.tfocal_transformer_hq import TemporalFocalTransformerBlock, SoftSplit, SoftComp
from model.inpainter.spectral_norm import spectral_norm as _spectral_norm
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.ModuleList([
            ...
            nn.LeakyReLU(0.2, inplace=False),
            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1),#最后是一个输出层
            nn.LeakyReLU(0.2, inplace=False),
    ...
    def forward(self, x):
        ...
        # 【关键修复】确保 x0, h, w 在循环开始前具备 Tensor/Int 类型
        # 使用 x 的一个克隆作为占位符，确保 x0 初始类型是 Tensor
        # clone().detach() 可以创建一个占位符，避免参与反向传播，且类型正确。
        x0 = x.clone().detach() 
        ...
        for i, layer in enumerate(self.layers):
            ...
            # 此时，x0 已经是 Tensor 类型，重新赋值 x0 = out 不会改变其类型
            if i == 8:
                x0 = out 
                _, _, h, w = x0.size() 
                
            if i > 8 and i % 2 == 0:
                # 确保 x0, h, w 在使用时已从循环外部初始化/在 i=8 处被正确赋值
                g = self.group[(i - 8) // 2]
                
                # 这里的 reshape 依赖于 i=8 时的 h, w
                x = x0.reshape(bt, g, -1, h, w)
                o = out.reshape(bt, g, -1, h, w)
                out = torch.cat([x, o], 2).reshape(bt, -1, h, w)
                
            out = layer(out)
        return out

class deconv(nn.Module):#一个反卷积的操作层
    def __init__(self,
                 input_channel,
                 output_channel,
                 kernel_size=3,
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=False),
            nn.Conv2d(64, 3, kernel_size=3, stride=1, padding=1))#解码输出
    ...
    def forward_bidirect_flow(self, masked_local_frames):
        ...
        masked_local_frames = F.interpolate(masked_local_frames.reshape(
            -1, c, h, w),
                                            scale_factor=1 / 4,
                                            mode='bilinear',
                                            align_corners=True,
                                            recompute_scale_factor=True)
        masked_local_frames = masked_local_frames.reshape(b, l_t, c, h // 4,
                                                       w // 4)
        ...
        pred_flows_forward = pred_flows_forward.reshape(b, l_t - 1, 2, h // 4,
                                                     w // 4)
        pred_flows_backward = pred_flows_backward.reshape(b, l_t - 1, 2, h // 4,
                                                       w // 4)
    ...
    def forward(self, masked_frames, num_local_frames):
        ...
        enc_feat = self.encoder(masked_frames.reshape(b * t, ori_c, ori_h, ori_w))
        _, c, h, w = enc_feat.size()
        fold_output_size = (h, w)
        local_feat = enc_feat.reshape(b, t, c, h, w)[:, :l_t, ...]
        ref_feat = enc_feat.reshape(b, t, c, h, w)[:, l_t:, ...]
        ...
        trans_feat = self.ss(enc_feat.reshape(-1, c, h, w), b, fold_output_size)
        ...
        output = self.decoder(enc_feat.reshape(b * t, c, h, w))
