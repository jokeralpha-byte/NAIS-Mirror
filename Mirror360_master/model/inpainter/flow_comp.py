...
class FlowCompletionLoss(nn.Module):
    ...
    def forward(self, pred_flows, gt_local_frames):
        ...
        with torch.no_grad():
            ...
            gt_local_frames = F.interpolate(gt_local_frames.reshape(-1, c, h, w),
                                            scale_factor=1 / 4,
                                            mode='bilinear',
                                            align_corners=True,
                                            recompute_scale_factor=True)
            gt_local_frames = gt_local_frames.reshape(b, l_t, c, h // 4, w // 4)
        ...
        forward_flow_loss = self.l1_criterion(
            pred_flows[0].reshape(-1, 2, h // 4, w // 4), gt_flows_forward)
        backward_flow_loss = self.l1_criterion(
            pred_flows[1].reshape(-1, 2, h // 4, w // 4), gt_flows_backward)
...
class SPyNet(nn.Module):
    ...
    def __init__(
        self,
        use_pretrain=True,
        pretrained='/data2/mmvisitor/Jia_Daiang/360_video_compression/SpectraMind360/checkpoint/spynet_20210409-c6c1bd09.pth'
    ):
        super().__init__()
